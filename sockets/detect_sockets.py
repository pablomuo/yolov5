#enviamos el stream con Gstreamer y enviamos los datos con SOCKETS
...
def run(
    ...
    #------------------------------------------------------------------------------------------------------------------------------------------------    sed the information to the client
    gst_str_rtp = " appsrc ! videoconvert ! videoscale ! video/x-raw,format=I420,width=1280,height=720,framerate=20/1 !  videoconvert !\
         ...
         udpsink host=10.236.25.41 port=8000"
    ...
    out_send = cv2.VideoWriter(gst_str_rtp, fourcc, 20, (frame_width, frame_height), True) 
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    for c in det[:, -1].unique():
                        ...
                        tagPrint= names[int(c)]
                        tagPrintNumber=(f"{n} : {names[int(c)]}")
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            if (c != 0):
                                ...
                                print(f' {n} : {names[int(c)]} p:{det[d, 4]:.4f}')
                                msg_para_enviar = f' {n} : {names[int(c)]} p:{det[d, 4]:.4f}\n'
                                

                            else: 
                            #     if (area1[d] > 106800):
                                    annotator.box_label(xyxy, label, color=colors(c, True))  
                                    print(f' {n} : {names[int(c)]} p:{det[d, 4]:.4f} area:{area1[d]}')
                                    msg_para_enviar = f' {n} : {names[int(c)]} p:{det[d, 4]:.4f} area:{area1[d]}\n'
                            ...
                            d = d+1
                            ##-------------------------------------------------------------------------------------------------------------------------------------------
                            #al poner el de arriba, la linea de abajo se comenta
                            #annotator.box_label(xyxy, label, color=colors(c, True)) 
                        ...
                        if mantener_el_socket_abierto:

                            dato_para_enviar_str = msg_para_enviar
